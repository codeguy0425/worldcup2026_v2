...
export function MatchPage() {
  ...
  // Build scorer name lookup from per-team ZH squad data (replaces scorer-names.json)
  const zhScorerMap = new Map<string, string>()
  if (curMatch) {
    if (team1Zh) for (const p of team1Zh) zhScorerMap.set(curMatch.team1Id + ':' + p.no, p.name)
    if (team2Zh) for (const p of team2Zh) zhScorerMap.set(curMatch.team2Id + ':' + p.no, p.name)
  }
  ...
  const nextRoundInfo = (() => {
    ...
    if (m.stage === 'sf') {
      ...
      const winnerTeam = finalMatch ? (() => {
        const isT1 = finalMatch.team1Original === wId
        const wid = isT1 ? finalMatch.team1Id : finalMatch.team2Id
        const w = teamMap.get(wid)
        return w ? `${w.flag} ${w.name}` : wid
      })() : null
      const loserTeam = thirdMatch ? (() => {
        const isT1 = thirdMatch.team1Original === lId
        const lid = isT1 ? thirdMatch.team1Id : thirdMatch.team2Id
        const l = teamMap.get(lid)
        return l ? `${l.flag} ${l.name}` : lid
      })() : null
      return {
        type: 'sf',
        ...
        {nextRoundInfo && nextRoundInfo.type === 'sf' && bracketData && (() => {
          ...
          return (
            <div style={{ marginTop: '14px', fontSize: '11px', color: 'var(--text-muted)', lineHeight: 1.8 }}>
              <div><span style={{ fontWeight: 600, color: '#34d399' }}>{nextRoundInfo.winnerTeam || t.match.winner}</span> → {nextRoundInfo.winnerRound}{nextRoundInfo.winnerOpp ? ` vs ${nextRoundInfo.winnerOpp}` : ''}</div>
              {fm && (
              ...
              <div><span style={{ fontWeight: 600, color: '#fb7185' }}>{nextRoundInfo.loserTeam || t.match.loser}</span> → {nextRoundInfo.loserRound}{nextRoundInfo.loserOpp ? ` vs ${nextRoundInfo.loserOpp}` : ''}</div>
              {tm && (
              ...
              {m.goals!.map((g, i) => {
                ...
                return (
                  <span key={i} style={{
                    ...
                    zIndex: 2,
                  }} title={`${(() => { const lang2 = t.lang === 'En' ? 'zh' : 'en'; if (lang2 === 'zh') { const n = g.scorerNo !== undefined ? zhScorerMap.get((g.ownGoal ? (g.teamId === m.team1Id ? m.team2Id : m.team1Id) : g.teamId) + ':' + g.scorerNo) : null; if (n) return n; } return g.scorer; })()} ${g.minute}'`} />
                )
              })}
              {/* Minute labels */}
              {(() => {
              ...
              <div style={{ textAlign: 'left' }}>
                {m.goals!.filter(g => (g.ownGoal ? g.teamId === m.team2Id : g.teamId === m.team1Id)).map((g, i) => (
                  <div key={i} style={{ marginBottom: '3px' }}>
                    ...
                    <span style={{ color: 'var(--text)' }}> {(() => { const lang = t.lang === 'En' ? 'zh' : 'en'; if (lang === 'zh') { const n = g.scorerNo !== undefined ? zhScorerMap.get((g.ownGoal ? (g.teamId === m.team1Id ? m.team2Id : m.team1Id) : g.teamId) + ':' + g.scorerNo) : null; if (n) return n; } return g.scorer; })()}{g.scorerNo !== undefined ? <span style={{ fontSize: '9px', color: 'var(--text-muted)' }}> #{g.scorerNo}</span> : ''}</span>
                    ...
                    {kicks.map((k: {player: string; no?: number; scored: boolean}, i: number) => {
                      ...
                      return (
                        <div key={i} style={{ lineHeight: 1.8 }}>
                          ...
                          {k.no && <span style={{ fontSize: '9px', color: 'var(--text-muted)', marginLeft: '2px' }}>#{k.no}</span>}
                        </div>
                      )
                    })}
                  </div>
                    ...
                    {detail.substitutions.map((s, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          <span style={{ color: 'var(--text-muted)', fontSize: '9px' }}>↓{pn(s.teamId, s.off.no)} <span style={{ fontFamily: 'var(--font-mono)' }}>#{s.off.no}</span></span>
                          <span style={{ color: 'var(--text-muted)' }}>→</span>
                          <span style={{ fontWeight: 500 }}>{pn(s.teamId, s.on.no)}</span>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-muted)' }}>#{s.on.no}</span>
