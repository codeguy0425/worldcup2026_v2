          ...
          {latest.map(m => {
            ...
            return (
              <Link key={m.id} to={`/match/${m.id}`} style={{
                ...
                <span style={{ color: 'var(--text-muted)', minWidth: '42px', fontSize: '9px', display: 'flex', alignItems: 'center', gap: '2px', position: 'relative' }}><span>{(() => { const lang = t.lang === 'En' ? 'zh' : 'en'; return hktDateLabel(m.date, m.timeUtc, lang) })()}</span>{viutvIds.has(m.id) && <span style={{ position: 'absolute', right: '-30px', top: '50%', transform: 'translateY(-50%)', lineHeight: 1 }}>📺</span>}</span>
