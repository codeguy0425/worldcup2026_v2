...
function main() {
  ...
  // 3b. Re-write matches.json with resolved team IDs for knockout matches
  const resolvedMap = {}
  for (const [stage, bMs] of Object.entries(bracket.rounds)) {
    for (const bm of bMs) {
  ...
  for (const m of cleanMatches) {
    if (resolvedMap[m.id]) {
      m.team1Id = resolvedMap[m.id].team1Id
      m.team2Id = resolvedMap[m.id].team2Id
    }
  }
  writeJSON(resolve(DATA_DIR, 'matches.json'), cleanMatches)
  console.log(`   ✅ matches.json (updated with resolved KO team IDs) — ${cleanMatches.length} matches`)
